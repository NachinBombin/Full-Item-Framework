-- ============================================================
--  NPC Flashbang Throw  |  npc_flashbang_throw.lua
--  Server-side only.
--
--  Combine soldiers, metrocops, and elites periodically have a
--  chance to lob a flashbang grenade (entity: ent_flashbang)
...
--
--  Requires: the ent_flashbang entity to be registered.
-- ============================================================
...
local cv_enabled    = CreateConVar("npc_flashbang_throw_enabled",    "1",    SHARED_FLAGS, "Enable/disable NPC flashbang throws.")
local cv_chance     = CreateConVar("npc_flashbang_throw_chance",     "0.20", SHARED_FLAGS, "Probability (0–1) that an eligible NPC throws a flashbang each check.")
local cv_interval   = CreateConVar("npc_flashbang_throw_interval",   "8",    SHARED_FLAGS, "Seconds between throw-eligibility checks per NPC.")
local cv_cooldown   = CreateConVar("npc_flashbang_throw_cooldown",   "18",   SHARED_FLAGS, "Minimum seconds between throws for the same NPC.")
local cv_speed      = CreateConVar("npc_flashbang_throw_speed",      "700",  SHARED_FLAGS, "Launch speed of the flashbang (units/s).")
local cv_arc        = CreateConVar("npc_flashbang_throw_arc",        "0.25", SHARED_FLAGS, "Upward arc factor (0 = flat, higher = more lob).")
local cv_spawn_dist = CreateConVar("npc_flashbang_throw_spawn_dist", "52",   SHARED_FLAGS, "Forward distance from NPC origin to spawn grenade (avoids self-collision).")
local cv_max_dist   = CreateConVar("npc_flashbang_throw_max_dist",   "2200", SHARED_FLAGS, "Max distance to player for a throw to be attempted.")
local cv_min_dist   = CreateConVar("npc_flashbang_throw_min_dist",   "120",  SHARED_FLAGS, "Min distance to player (no throw if closer than this).")
local cv_spin       = CreateConVar("npc_flashbang_throw_spin",       "1",    SHARED_FLAGS, "Apply a random spin impulse to the grenade (1 = enabled).")
local cv_announce   = CreateConVar("npc_flashbang_throw_announce",   "0",    SHARED_FLAGS, "Print a debug message to console each time an NPC throws.")

-- ============================================================
--  NPC whitelist
-- ============================================================

local FLASH_THROWERS = {
...
local function IsEligibleThrower(npc)
    ...
    return FLASH_THROWERS[npc:GetClass()] == true
end
...
--- launches ent_flashbang exactly 1 second later.
---@param npc Entity  The throwing NPC.
---@param target Entity  The target (player).
---@return boolean  true on success, false on failure.
local function ThrowFlashbang(npc, target)
    ...
    timer.Simple(1, function()
        ...
        -- Create the flashbang entity.
        local flash = ents.Create("ent_flashbang")
        if not IsValid(flash) then
            if cv_announce:GetBool() then
                print("[NPC Flashbang Throw] ERROR: ent_flashbang entity could not be created. Is the required addon installed?")
            end
            return
        end
        ...
        flash:SetPos(spawnPos + right * 6 + up * -2)
        flash:SetAngles(npc:GetAngles() + Angle(-90, 0, 0))
        ...
        local phys = flash:GetPhysicsObject()
        if IsValid(phys) then
            ...
            if cv_spin:GetBool() then
                ...
                local offset = flash:LocalToWorld(flash:OBBCenter())
        ...
        if cv_announce:GetBool() then
            print(string.format("[NPC Flashbang Throw] %s threw a flashbang at %s (dist at trigger: %.0f)",
...
local function InitNPCState(npc)
    ...
    if npc.__flash_hooked then return end
    npc.__flash_hooked    = true
    npc.__flash_nextCheck = CurTime() + math.Rand(1, cv_interval:GetFloat())
    npc.__flash_lastThrow = 0
end
...
timer.Create("NPCFlashbangThrow_Think", 0.5, 0, function()
    ...
    for _, npc in ipairs(ents.GetAll()) do
        ...
        if now < (npc.__flash_nextCheck or 0) then continue end
        npc.__flash_nextCheck = now + interval + math.Rand(-1, 1)

        -- Cooldown gate
        if now - (npc.__flash_lastThrow or 0) < cooldown then continue end
...
hook.Add("InitPostEntity", "NPCFlashbangThrow_Init", function()
    print("[NPC Flashbang Throw] Addon loaded.")
    print("[NPC Flashbang Throw] Use 'npc_flashbang_throw_*' convars to configure.")
    print("[NPC Flashbang Throw] Requires the ent_flashbang entity.")
end)
